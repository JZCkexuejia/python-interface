...
from atrial import AtrialVentricular
from handlers import ConnectionHandler, GraphsHandler
...
class SettingsWidget(QWidget):  # 子类，创建设置窗口
    ...
    def __init__(self, parent=None):
        ...
        # 信号连接槽函数
        self.ui.pushButton_open_serial.clicked.connect(self.open_atrial)
        self.ui.pushButton_refresh.clicked.connect(self.refresh_port)
        self.ui.send_serial_btn.clicked.connect(self.send_serial)
        self.__serial = None

        # 显示心房和心室的图
        self.atrial = AtrialVentricular(None)
        self._graphs = GraphsHandler(self.atrial.ui.atrial_plots, self.atrial.ui.vent_plots, data_size=2001)
        self._conn = ConnectionHandler()
        self._conn.serial.ecg_data_update.connect(self._graphs.update_data)
        # self._conn.serial.connect('COM3')
        # self._conn.serial._try_to_open_port()
    ...
    def send_serial(self):
        ...
        # 发送到pacemaker
        self._conn.send_data_to_pacemaker(content)
    ...
    def open_atrial(self):
        print('open atrial and ventricular')
        a = AtrialVentricular(self)
        a.show()
